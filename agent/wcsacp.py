...
class WCSACPAgent(Agent):
    ...
    def __init__(self, obs_dim, action_dim, action_range, device, critic_cfg, safety_critic_cfg,
                 ...
                 risk_level, damp_scale, lr_scale, epsilon, smooth_lambda):
        super().__init__()
    ...
    def update_actor_and_alpha_and_beta(self, obs, action_taken, logger, step):
        ...
        # Actor Smoothing
        noise_dist = tdist.multivariate_normal.MultivariateNormal(loc=torch.zeros(obs.shape[1]), covariance_matrix=torch.eye(obs.shape[1]))
        noise = noise_dist.rsample(torch.randn(obs.shape[0]).shape)
        noise = noise / torch.norm(noise, p=2, dim=1).view(obs.shape[0], 1)
        noise = noise * self.epsilon
        noise = noise * torch.rand(1)
        obs_noisy = obs + noise.cuda()
        dist2 = self.actor(obs_noisy)
        smooth_loss = torch.mean(0.5 * (tdist.kl.kl_divergence(dist, dist2) + tdist.kl.kl_divergence(dist2, dist)))
        ...
        logger.log('train/smooth_loss', smooth_loss, step)
