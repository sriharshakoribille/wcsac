...
class SACLAGAgent(Agent):
    ...
    def update_actor_and_alpha_and_beta(self, obs, action_taken, logger, step):
        ...
        logger.log('train/actor_cost', torch.mean(actor_QC), step)
        ...
        if self.learnable_temperature:
            ...
            beta_loss = torch.mean(self.beta * (self.target_cost - current_QC).detach())
    ...
    def save(self, path):
        ...
        torch.save(self.safety_critic.QC1.state_dict(), os.path.join(path, 'safety_critic_QC1.pt'))
        torch.save(self.safety_critic.QC2.state_dict(), os.path.join(path, 'safety_critic_QC2.pt'))
    ...
    def load(self, path):
        ...
        self.safety_critic.QC1.load_state_dict(torch.load(os.path.join(path, 'safety_critic_QC1.pt')))
        self.safety_critic.QC2.load_state_dict(torch.load(os.path.join(path, 'safety_critic_QC2.pt')))
