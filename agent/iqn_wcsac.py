...
class WCSACAgent(Agent):
    ...
    def __init__(
        ...
        # learnable quantiles
        self.num_quantiles = 8  # number of quantiles to use during training
    ...
    def update_critic(self, obs, action, reward, cost, next_obs, not_done, logger, step):
        ...
        current_QC, current_QC_taus = self.safety_critic(obs, action)  # quantile distribution of cost

        # QC = quantile distribution target
        next_QC, _ = self.safety_critic_target(next_obs, next_action)  # (bs, N, 1)
        next_QC = next_QC.transpose(1,2)
        ...
        # Safety Critic Loss
        safety_critic_loss = self.quantile_huber_loss(td_error_QC, taus=current_QC_taus)
    ...
    def update_actor_and_alpha_and_beta(self, obs, action_taken, logger, step):
        ...
        actor_QC, _ = self.safety_critic(obs, action, num_tau=100, risk_level=self.risk_level)

        # Safety Critic with actual actions
        current_QC, _ = self.safety_critic(obs, action_taken, num_tau=100, risk_level=self.risk_level)

        # CVaR + Damp impact of safety constraint in actor update / not used if damp_scale = 0
        cvar_damp = current_QC.squeeze(-1).mean(dim=-1, keep_dim=True)  # approximate cvar
        damp = self.damp_scale * torch.mean(self.target_cost - cvar_damp)
        cvar = actor_QC.squeeze(-1).mean(dim=-1, keep_dim=True)
    ...
    def quantile_huber_loss(self, td_errors, taus, k=1.0, n=8):
        """
